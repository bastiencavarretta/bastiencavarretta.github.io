...
_Référentiel :_ Terrestre Galiléen.

_Coordonnées :_ Cartésiennes $(O,\vec{e_x},\vec{e_z})$ ou O est le bas de la corde.
...
_Masse :_ On note $\gamma=\frac{m}{l}$ la densité linéique de la corde. Par Pythagore, $\d m = \gamma \| \overrightarrow{\d M} \|^2 = \gamma \sqrt{1 + \dot h(x)^2}\d x$.

_Bilan des forces:_ Gravité $-\d m g \vec{e_z} = $. Tension de la section de droite $\vec T(x+dx)$. Tension de la section de gauche $-\vec T(x)$. Le PFD à l'équilibre donne
